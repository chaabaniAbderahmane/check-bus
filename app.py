...
def show_admin_login():
    ...
    with st.expander("➕ Créer mon premier voyage"):
        with st.form("first_trip"):
            ...
            trip_name = st.text_input("Nom du voyage", key="ft_name")
            destination = st.text_input("Destination", key="ft_dest")
            date = st.date_input("Date", key="ft_date")
            bus = st.text_input("Numéro de bus", key="ft_bus")
            
            admin_first = st.text_input("Votre prénom", key="ft_af")
            admin_last = st.text_input("Votre nom", key="ft_al")
            admin_user = st.text_input("Nom d'utilisateur admin", key="ft_au")
            admin_pass = st.text_input("Mot de passe admin", type="password", key="ft_ap")
...
def show_qr_scanner():
    ...
    st.markdown("""
    <div style="border: 3px dashed #FF6B35; border-radius: 20px; padding: 40px; text-align: center; background: rgba(255,107,53,0.05);">
